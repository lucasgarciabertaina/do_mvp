...
const seedUsersString: string | undefined = process.env.SEED_USERS;
...
async function main() {
    if (seedUsersString) {
      try {
        // Convierte el string de la variable de entorno a un array de objetos
        const usersSeed: SeedUser[] = JSON.parse(seedUsersString);
        
        console.log(`Cargando ${usersSeed.length} usuarios...`);
    ...
    console.log('DateVotes seeded successfully: 1 vote total (by Mondi for Option 3).')
        ...
        console.error("Error: La variable SEED_USERS no tiene un formato JSON válido.", error);
      }
    } else {
        console.warn("La variable SEED_USERS no está definida.");
    }
}
